...
committed misses nothing, and an idle group never reads as stale. Verified
against both real exports on hand (2026-07-09): the reported incident (three
engines dark at epochs 11–14 of 16, one active engine cut off from commits for
hours) and exp-07 (one engine eighteen hours behind the other's epoch 17) —
**both previously classified `Healthy`**; exp-07's "healthy" label in the plan
doc predates this gate and was a false negative.

Two gate designs were evaluated against real exports and deliberately
**rejected**:
...
- *Member-count coverage* (`group_context.member_count` > exporting engines):
  ...
  (exp-07: six members, two engines) — the gate would quarantine them all,
  permanently, and its precedence masked the sharper per-engine signals on the
  real incident.
